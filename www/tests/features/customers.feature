Feature: Customers
  ...
  Scenario: Create new customer
    ...
    And the user types "Client" in the customer-field
    When the user presses the "Neuen Kunden anlegen"-Button
    Then the customer must be seen in the table
  ...
  Scenario: There is already a customer with this name
    ...
    And no changes appear in the table

  Scenario: Search
    Given there are customers with name "Client_1", "Client_2", "Another_1", "Another_2" 
    And the user types "Client" in the customer-field
    When the user presses the search-Button
    Then only the customers "Client_1" and "Client_2" should be seen in the table

  Scenario: Delete Customer
    Given there is a customer with name "Client" in the table
    And the user presses the trash-button
    Then the customer should be deleted from the table
